...
L’architettura del tool è organizzata come una pipeline di trasformazioni successive che partono dal codice sorgente del programma e producono rappresentazioni via via più strutturate. Il flusso generale del sistema è rappresentato dal seguente schema:
...
L’AST non viene generato direttamente dal parser. Il parser di ANTLR produce infatti un \textit{parse tree} che riflette la grammatica del linguaggio ma la costruzione dell’AST consiste in una trasformazione a partire dal parse tree.
...
Il tool definisce una classe \texttt{AstBuilderVisitor} che attraversa ricorsivamente il parse tree e costruisce i nodi dell’AST.
...
Da notare come durante la costruzione dell’AST vengono propagate le informazioni di localizzazione del sorgente. Ogni nodo contiene infatti un campo \texttt{SourceRange} che registra la posizione della corrispondente porzione di codice nel file originale. Questa informazione non è necessaria per la semantica del linguaggio ma  è utile perchèp consente di produrre messaggi di errore più precisi nelle fasi successive del tool.
...
Viene verificato che la struttura del programma sia completa dal punto di vista esecutivo, ad esempio controllando la presenza del blocco \texttt{main}, che rappresenta il punto di ingresso dell’esecuzione.
